  ...
  return (
    <div className="p-6 space-y-6 max-w-2xl mx-auto pb-32">
      ...
      <header className="flex flex-col items-center text-center pt-4 pb-2">
         <h1 className={`${cinzel.className} text-3xl md:text-4xl text-[#E0E0E0] drop-shadow-lg tracking-wide`}>
            EMILY MATOS
         </h1>
      ...
      {/* BARRA DE DATA FLUTUANTE (Ajustada) */}
      <div className="sticky top-4 z-40 bg-white/10 backdrop-blur-md p-2 rounded-[2rem] border border-white/20 shadow-xl flex justify-between items-center gap-2 pr-2 pl-6">
        <div className="flex items-center gap-3 text-white flex-1">
            ...
            <input 
                ...
                className="font-bold text-white outline-none bg-transparent uppercase text-sm tracking-widest cursor-pointer w-full" 
            />
        </div>
        ...
        {agendamentos.length === 0 && (
            <div className="text-center py-20 flex flex-col items-center gap-4 opacity-50">
                <div className="p-6 rounded-full bg-white/5 border border-white/10">
                    <CalendarIcon size={32} className="text-[#D49FAF]" />
                </div>
          ...
          <div key={ag.id} className={`relative p-5 rounded-[2rem] bg-[#E0E0E0] flex flex-col gap-4 shadow-lg transition-all border-l-4 ${ag.status === 'concluido' ? 'border-green-400 opacity-80 grayscale-[0.5]' : 'border-[#A16585]'}`}>
            ...
            {ag.status !== 'concluido' && (
                <div className="flex gap-2 justify-end border-t border-[#373F47]/10 pt-3 mt-1">
                    ...
                    <button onClick={() => setOpcoesAg(ag)} className="px-4 py-2 rounded-xl bg-white text-[#373F47] hover:bg-red-50 text-[10px] font-bold uppercase shadow-sm">
                         Opções
                    </button>
                    <button onClick={() => {
        ...
        <div className="fixed inset-0 bg-[#373F47]/80 backdrop-blur-sm flex items-center justify-center p-6 z-[60]">
           <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl max-w-sm w-full relative animate-in zoom-in-95">
              ...
              <div className="text-center mb-6 mt-2">
                  <div className="inline-flex items-center justify-center w-16 h-16 bg-green-100 rounded-full text-green-600 mb-3 shadow-sm">
                  ...
                  {modalMensagem.status !== 'concluido' ? (
                    <>
                        <button onClick={() => gerarMensagem('confirma')} className="p-4 rounded-2xl bg-gray-50 hover:bg-[#F7ACCF]/10 border border-gray-100 hover:border-[#F7ACCF] transition-all flex items-center gap-4 text-left group">
                            ...
                            <div>
                                <p className="text-sm font-bold text-[#373F47]">Confirmar</p>
                                <p className="text-[10px] text-gray-400">Enviar lembrete do horário</p>
                            </div>
                        </button>
                    ...
                    <button onClick={() => gerarMensagem('posvenda')} className="p-4 rounded-2xl bg-purple-50 border border-purple-100 flex items-center gap-4 text-left group">
                        ...
                        <div>
                            <p className="text-sm font-bold text-[#373F47]">Pós-Venda</p>
                            <p className="text-[10px] text-gray-400">Coletar feedback da cliente</p>
                        </div>
                    </button>
                  )}
              </div>
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-[#373F47]/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl max-w-sm w-full border-t-8 border-[#F7ACCF] animate-in slide-in-from-bottom-10">
            <h2 className={`${cinzel.className} text-xl text-center mb-6 text-[#373F47] font-bold`}>NOVO CLIENTE</h2>
            ...
            <form onSubmit={async (e) => { 
              ...
              <div className="space-y-4">
                  ...
                  <input placeholder="WhatsApp" value={telefone} onChange={e => setTelefone(e.target.value)} className="w-full p-4 bg-[#F5F5F5] text-[#373F47] rounded-2xl outline-none font-bold text-sm focus:ring-2 focus:ring-[#F7ACCF] transition-all" required />
                      ...
                      <div className="absolute z-50 w-full bg-white border border-gray-100 mt-2 rounded-2xl shadow-xl max-h-48 overflow-y-auto p-1">
                        {servicosBase.map(s => (
                          <button key={s.id} type="button" onClick={() => { setServico(s.nome); setPrecoSugerido(s.preco_base); setCustoSugerido(s.custo_material_base); setMostrarListaServicos(false); }} className="w-full text-left px-4 py-3 hover:bg-[#F7ACCF]/10 rounded-xl text-[#373F47] text-xs border-b border-gray-50 last:border-none font-bold flex items-center justify-between group">
                              <span>{s.nome}</span>
                              <span className="text-[#A16585] group-hover:text-[#373F47]">R$ {s.preco_base}</span>
                          </button>
                        ))}
                      </div>
                    )}
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" disabled={isEnviando} className="flex-[2] py-4 bg-[#373F47] text-white rounded-2xl font-bold text-xs uppercase tracking-widest shadow-lg hover:bg-black transition-all">{isEnviando ? "..." : "Confirmar"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {finalizandoAg && (
        <div className="fixed inset-0 bg-[#373F47]/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl max-w-sm w-full border-t-[8px] border-green-400 animate-in zoom-in-95">
            ...
            <form onSubmit={concluirFinanceiro} className="space-y-4">
              <div className="space-y-1">
                 ...
                 <input type="number" step="0.01" value={finalizandoAg.valor_total} onChange={e => setFinalizandoAg({...finalizandoAg, valor_total: e.target.value})} className="w-full p-4 bg-[#F5F5F5] text-[#373F47] rounded-2xl outline-none font-bold text-2xl text-center focus:ring-2 focus:ring-green-200" required />
              </div>
              
              <div className="flex gap-3">
                <div className="flex-1 space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-2">Custo</label>
                    <input type="number" step="0.01" value={finalizandoAg.custo_material} onChange={e => setFinalizandoAg({...finalizandoAg, custo_material: e.target.value})} className="w-full p-3 bg-[#F5F5F5] text-gray-400 rounded-xl outline-none text-sm font-bold text-center" />
                </div>
                <div className="w-24 space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-2">Comissão %</label>
                    <input type="number" value={finalizandoAg.porcentagem_comissao} onChange={e => setFinalizandoAg({...finalizandoAg, porcentagem_comissao: e.target.value})} className="w-full p-3 bg-[#F5F5F5] rounded-xl outline-none font-bold text-[#A16585] text-sm text-center" />
                </div>
              </div>
              ...
              <div className="bg-[#373F47] p-4 rounded-2xl text-center shadow-lg mt-2">
                  ...
                  <p className="text-2xl font-serif text-[#F7ACCF]">R$ {(( (parseFloat(finalizandoAg.valor_total) || 0) - (parseFloat(finalizandoAg.custo_material) || 0) ) * ((parseFloat(finalizandoAg.porcentagem_comissao) || 100) / 100)).toFixed(2)}</p>
              </div>
      ...
      {opcoesAg && (
        <div className="fixed inset-0 bg-[#373F47]/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl max-w-xs w-full text-center animate-in zoom-in-95">
            ...
            <div className="space-y-3">
              ...
              <button onClick={async () => { if(confirm("Tem certeza que deseja cancelar?")) { await supabase.from("agendamentos").update({ status: 'cancelado' }).eq('id', opcoesAg.id); setOpcoesAg(null); buscarAgendamentos(); } }} className="w-full py-4 bg-[#F5F5F5] text-red-400 rounded-2xl font-bold text-sm hover:bg-red-50">Cancelar Agendamento</button>
              <button onClick={() => setOpcoesAg(null)} className="text-gray-300 font-bold text-[10px] uppercase mt-4 block hover:text-gray-500">Voltar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {remarcandoAg && (
        <div className="fixed inset-0 bg-[#373F47]/80 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl max-w-xs w-full text-center">
